...
var_dump($opts);  //add it by Eric
    ...
    try {
      if ($curl = curl_init()) {
var_dump($curl); //add it by Eric
        if (curl_setopt_array($curl, $opts)) {
var_dump($opts);
          if ($response = curl_exec($curl)) {
//var_dump($response); //add it by Eric
            $parts = explode("\r\n\r\n", $response, 3);
            ...
            return array($status, $headers, $body);
              ...
              throw new Services_Twilio_TinyHttpException(curl_error($curl));  // here !!!
          }
